...
def object_detector(image):
    ...
    for (classid, score, box) in zip(classes, scores, boxes):
        ...
        # Getting the data
        # 1: Class name, 2: Object width in pixels, 3: Position where the text should be drawn (distance)
        if classid == 0:  # Person class ID
            data_list.append([class_names[classid[0]], box[2], (box[0], box[1] - 2)])
        elif classid == 67:  # Mobile class ID
            data_list.append([class_names[classid[0]], box[2], (box[0], box[1] - 2)])
        # If you want to include more classes, you can simply add more 'elif' statements here.
...
while True:
    ...
    for d in data:
        ...
        cv.putText(frame, f'Distance: {round(distance, 2)} inches', (x + 5, y + 13), FONTS, 0.48, GREEN, 2)

        # Convert the text to speech
        speak(f'{object_name}: Distance {round(distance, 2)} inches')
